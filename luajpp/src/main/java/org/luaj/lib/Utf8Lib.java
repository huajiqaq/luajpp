...
public class Utf8Lib extends TwoArgFunction {
    ...
    public LuaValue call(LuaValue modname, LuaValue env) {
        ...
        if (LuaUtf8String.s_metatable == null) {
            LuaUtf8String.s_metatable = LuaValue.tableOf(new LuaValue[]{LuaConstants.INDEX, string});
        }
        return string;
    }
    ...
    static class MatchState {
        ...
        static boolean match_class(int c, int cl) {
            ...
            switch (lcl) {
                ...
                case 'c':
                    ...
                    res = Character.isISOControl(c);
                    break;
                case 'p':
                    //res = cdata<255&&(cdata & MASK_PUNCT) != 0;
                    res = isPunctuation(c);
                    break;
                case 's':
                    //res = (cdata & MASK_SPACE) != 0;
                    res = Character.isWhitespace(c);
                    break;
                case 'g':
                ...
                case 'w':
                    ...
                    res = Character.isLetterOrDigit(c);
                    break;
                case 'x':
                    res = cdata < 255 && (cdata & MASK_HEXDIGIT) != 0;
                    break;
                case 'z':
                    res = (c == 0);
        ...
        public static boolean isPunctuation(int c) {
            return (c < 0x20 && c != 0x7F) || (c >= 0x2E && c <= 0x3F) || (c >= 0x41 && c <= 0x5A)
                    || (c >= 0x61 && c <= 0x7A) || (c >= 0x80 && c <= 0x9F) || (c >= 0xA1 && c <= 0xBF)
                    || (c >= 0xD800 && c <= 0xDFFF && !Character.isSurrogate((char) c)) || (c >= 0xFDD0 && c <= 0xFDEF)
                    || (c & 0xFFFF) == 0xFDDF;
        }
