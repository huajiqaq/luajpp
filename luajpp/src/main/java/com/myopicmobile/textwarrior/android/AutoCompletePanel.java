...
public class AutoCompletePanel {
    ...
    private void setVerticalOffset(int verticalOffset) {
        ...
        int max = -_autoCompletePanel.getHeight();
                ...
                protected FilterResults performFiltering(CharSequence constraint) {
                    ...
                    if (ss.length == 2) {
                        ...
                        if (key.charAt(key.length() - 1) == '.') {
                            ...
                            int idx = _textField.getCaretPosition() - keyword.length() - 1;
                            /*if(idx==_textField.getLength())
                                idx=idx-keyword.length()-1;*/
                            buf.addAll(LuaParser.filterLocal(keyword, idx, _textField.getColorScheme()));
                            ...
                            ArrayList<String> plist = new ArrayList<>(LexState.packageList);
                            for (String s : plist) {
                                buf.addAll(PackageUtil.filter(s + keyword));
                            }
                        }
                    }
