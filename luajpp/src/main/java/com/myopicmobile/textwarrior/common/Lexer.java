...
public class Lexer {
    ...
    private static boolean isKeyword(LuaTokenTypes t) {
        return switch (t) {
            case TRUE, FALSE, DO, FUNCTION, NOT, AND, OR, IF, THEN, ELSEIF, ELSE, WHILE, FOR, IN,
                 RETURN, BREAK, LOCAL, REPEAT, UNTIL, END, NIL, SWITCH, CASE, DEFAULT, CONTINUE,
                 GOTO, LAMBDA, DEFER, WHEN -> true;
            default -> false;
        };
    }

    public void tokenize(DocumentProvider hDoc, Globals globals) {

        LexState.errormsg = null;
        if (!Lexer.getLanguage().isProgLang() || hDoc.length() == 0) {
            return;
        }
        ...
        cancelTokenize();
        try {
            _workerThread = new LexThread(this, globals);
            _workerThread.start();
        } catch (Exception e) {
            LuaLog.getInstance().addError("Lexer", e);
        }
    }
    ...
    private class LexThread extends Thread {
        ...
        public LexThread(Lexer p, Globals globals) {
        ...
        @Override
        public void run() {
            LuaConfig.log("lexerrun: start" + this);
            rescan = false;
            _abort.clear();
            try {
                ...
                LuaLog.getInstance().addError("LexThread", e);
                _tokens = new ArrayList<>();
                rescan = false;
            }
            LuaConfig.log("lexerrun: end" + this);
            ...
            if (rescan) {
                new LexThread(_lexManager, globals).start();
            }
        }
        ...
        private void tokenize() {
            ...
            if (rowCount < maxRow) {
                if (!LuaParser.lexer(new DocumentProvider(hDoc), globals, _abort) && _tokens != null && _tokens.size() > 4) {
            ...
            HashMap<String, ArrayList<Pair>> values = new HashMap<>(LexState.valueMap);
                ...
                while (!_abort.isSet()) {
                    ...
                    if (isModule && lastType == LuaTokenTypes.STRING && type != LuaTokenTypes.STRING) {
                        ...
                        if (bul.length() > 2) {
                            ...
                            LuaParser.addUserWord(na + " :require");
                        }
                        ...
                        case NAME:
                            ...
                            if (lastType != LuaTokenTypes.DOT) {
                                ...
                                /*if (locals.containsKey(name)) {
                                    ...
                                    for (Pair l : ls) {
                                        Log.i("lua", "lexr:1 "+name+":"+l.getFirst() +":"+ idx+":"+name.length());
                                        if (l.getFirst() == idx) {
                                            Pair p = tokens.get(tokens.size() - 1);
                                            p.setSecond(LOCAL);
                                            loc = true;
                                ...
                                if (values.containsKey(name)) {
                                    ...
                                    if (ls != null) {
                                        for (int i = ls.size() - 1; i >= 0; i--) {
                                            Pair l = ls.get(i);
                                            //Log.i("luaj", "lexr:2 "+name+":"+l.getFirst() +":"+ idx+":"+name.length());
                                            if (l.getFirst() == idx) {
                                                Pair p = tokens.get(tokens.size() - 1);
                                                int tp = l.getSecond();
