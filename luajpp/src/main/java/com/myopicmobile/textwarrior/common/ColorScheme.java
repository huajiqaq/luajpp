...
public abstract class ColorScheme {
    ...
    private final HashMap<Colorable, Integer> _colors = generateDefaultColors();
    ...
    public int getTokenColor(int tokenType) {
        Colorable element = switch (tokenType) {
            case Lexer.GLOBAL -> Colorable.GLOBAL;
            case Lexer.UPVAL -> Colorable.UPVAL;
            case Lexer.LOCAL -> Colorable.LOCAL;
            case Lexer.NORMAL -> Colorable.FOREGROUND;
            case Lexer.KEYWORD -> Colorable.KEYWORD;
            case Lexer.NAME -> Colorable.NAME; //fall-through
            case Lexer.DOUBLE_SYMBOL_LINE, Lexer.DOUBLE_SYMBOL_DELIMITED_MULTILINE,
                 Lexer.SINGLE_SYMBOL_LINE_B, Lexer.COMMENT -> Colorable.COMMENT; //fall-through
            case Lexer.STRING, Lexer.SINGLE_SYMBOL_DELIMITED_A, Lexer.SINGLE_SYMBOL_DELIMITED_B ->
                    Colorable.STRING;
            case Lexer.LITERAL -> Colorable.LITERAL; //fall-through
            case Lexer.SINGLE_SYMBOL_LINE_A, Lexer.SINGLE_SYMBOL_WORD, Lexer.OPERATOR ->
                    Colorable.SECONDARY;
            default -> {
                TextWarriorException.fail("Invalid token type");
                yield Colorable.FOREGROUND;
            }
        };
